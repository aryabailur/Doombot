...
# Strongest edges kept per issue. The floor bounds edge *quality* but not edge
# *count*: on a repository with repetitive reports every pair clears it, and the
# result is a hairball that neither renders smoothly nor communicates anything.
# 5 keeps each issue's closest relatives -- the question duplicate triage
# actually asks -- and is what holds a large repository under a thousand edges.
MAX_NEIGHBOURS = 5

# Largest label group that still earns "shared label" edges. Sharing a rare
# label is evidence; sharing a repo-wide one is not, and a group of n produces
# C(n, 2) edges, so this is what stops one common label generating a thousand.
MAX_LABEL_GROUP = 6
...
def build_graph(repo_name: str, security_numbers: set[int] | None = None) -> dict:
    ...
    # --- similarity edges ---------------------------------------------------
    #
    # Keep each issue's strongest MAX_NEIGHBOURS edges rather than every pair
    # above the floor.
    #
    # The floor alone does not bound the edge count, and on a repository whose
    # reports are genuinely repetitive it barely filters at all. Measured on
    # yt-dlp: 150 issues produced 9798 edges -- 65 per node, a 1.1 MB payload,
    # and roughly ten thousand line segments redrawn every animation frame,
    # which is what made the canvas lag. The median pair scored 0.784, well
    # above the 0.65 floor, because "youtube 403 error" reports really are all
    # alike.
    #
    # A k-nearest-neighbour graph keeps what the view is for -- each issue's
    # closest relatives, which is what duplicate triage actually asks about --
    # while dropping the long tail of weak pairs that only add ink. An edge
    # survives if either endpoint ranks it in its own top K, so a node is never
    # orphaned just because its best match is someone else's sixth best.
    ...
    for number, neighbours in candidates.items():
        neighbours.sort(reverse=True)
        for score, other in neighbours[:MAX_NEIGHBOURS]:
            kept[tuple(sorted((number, other)))] = score

    for (left, right), score in sorted(kept.items()):
        seen.add((left, right))
        links.append({
            "source": f"issue-{left}",
    ...
    for label, numbers in by_label.items():
        # Only *rare* shared labels earn an edge.
        #
        # The old ceiling was len(nodes) // 3, which sounds conservative and is
        # not: a label held by 49 of 150 issues passed it and then produced
        # C(49,2) = 1176 edges by itself. Measured on yt-dlp, metadata edges
        # were 1799 of 2385 links -- three quarters of the graph, generated by
        # a handful of common labels.
        #
        # The signal is inverted from the count. Two issues sharing a rare
        # label ("rocm", "geoblock") really are related; forty issues sharing
        # "site-bug" tell you only that this is a yt-dlp repository. A small
        # absolute ceiling keeps the informative case and drops the noise,
        # bounding each label at C(MAX_LABEL_GROUP, 2) edges.
        if len(numbers) < 2 or len(numbers) > MAX_LABEL_GROUP:
            continue
