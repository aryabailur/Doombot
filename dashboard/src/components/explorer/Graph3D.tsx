...
export interface Graph3DProps {
  ...
  isDark: boolean;
  pal: ExplorerPalette;
  onNodeClick: (node: CodeGraphNodeApi) => void;
  onNodeHover: (node: CodeGraphNodeApi | null) => void;
}
...
export default function Graph3D({
  ...
  const cityData = useMemo(() => {
    ...
    const byCluster = new Map<string, CodeGraphNodeApi[]>();
  ...
  const nodeThreeObject = useCallback(
    (raw: any) => {
      const node = raw as CodeGraphNodeApi & { __platformTop?: number };
  ...
  return (
    <ForceGraph3D
      ...
      onNodeClick={(raw: any) => onNodeClick(raw as CodeGraphNodeApi)}
      onNodeHover={(raw: any) => onNodeHover((raw as CodeGraphNodeApi) ?? null)}
