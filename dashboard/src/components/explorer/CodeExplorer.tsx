...
import type { CodeGraphNode, CodeGraphResponse } from "@/lib/types";
...
export function CodeExplorer({
  ...
  const [pathSource, setPathSource] = useState<CodeGraphNode | null>(null);
  const [pathTarget, setPathTarget] = useState<CodeGraphNode | null>(null);
  ...
  const selectSymbol = useCallback(
    (node: CodeGraphNode, options?: { openTab?: DetailTab }) => {
  ...
  const paintNode = useCallback(
    (raw: object, ctx: CanvasRenderingContext2D, globalScale: number) => {
      const node = raw as CodeGraphNode & { x?: number; y?: number };
      const x = node.x;
      const y = node.y;
          ...
          >
            <ForceGraph2D
              ...
              onNodeHover={(raw: object | null) =>
                setHoverId(raw ? (raw as CodeGraphNode).id : null)
              }
