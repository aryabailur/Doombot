...
import type { IssueGraphNodeApi, IssueGraphResponseApi } from "../../lib/types";
...
interface IssueExplorerProps {
  repoName: string;
  graph: IssueGraphResponseApi | null;
  loading: boolean;
  error: string | null;
  isDark: boolean;
  onToggleTheme: () => void;
...
export function IssueExplorer({
  ...
  const [selected, setSelected] = useState<IssueGraphNodeApi | null>(null);
  ...
  function focusIssue(node: IssueGraphNodeApi) {
    setSelected(node);
    setSelectedLinkKey(null);
    const onCanvas = data.nodes.find((candidate) => candidate.id === node.id) as
      | (IssueGraphNodeApi & { x?: number; y?: number })
      | undefined;
        ...
        {error ? (
          ...
          >
            <ForceGraph2D
              ...
              onNodeClick={(raw: object) => focusIssue(raw as IssueGraphNodeApi)}
              onNodeHover={(raw: object | null) =>
                setHoverId(raw ? (raw as IssueGraphNodeApi).id : null)
              }
              onBackgroundClick={() => {
                setSelected(null);
                setSelectedLinkKey(null);
